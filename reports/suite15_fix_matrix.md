...
- Baseline: 7/20 (35.0%)
- Avg response time: 37009.3 ms

## Top 5 Fixes (Prioritized)

| Rank | Fix ID | Impact | Cases | Effort |
| --- | --- | ---: | ---: | --- |
| 1 | F1_ROUTE_GUARD_NO_NULL_ROUTE | 35 | 7 | M |
| 2 | F3_SOURCE_ENFORCEMENT_FOR_SUMMARY | 12 | 4 | S |
| 3 | F4_INTERNAL_SOURCE_SANITIZATION | 12 | 3 | S |
| 4 | F5_TREATMENT_REFUSAL_PATH | 5 | 1 | S |
| 5 | F2_AMBIGUITY_CLARIFICATION_RENDERER | 4 | 1 | S |

### 1. Bloquer les réponses llm/evidence_template sans route déterministe validée (`F1_ROUTE_GUARD_NO_NULL_ROUTE`)
- Impact score: 35
- Affected cases: UNEXP_IS_URGENT, UNEXP_OUT_OF_RANGE, UNEXP_RENAL_NORMAL, UNEXP_TOX_POSITIVE, UNEXP_TSH_HOW, UNEXP_TSH_WHICH_REPORTS, UNEXP_WHICH_EXCEED
...
### 2. Forcer les sources cliquables dans clarifications/summaries déterministes (`F3_SOURCE_ENFORCEMENT_FOR_SUMMARY`)
- Impact score: 12
...
### 3. Sanitiser tous les formats internes de source (doc_id/chunk_id) côté texte (`F4_INTERNAL_SOURCE_SANITIZATION`)
- Impact score: 12
- Affected cases: UNEXP_OUT_OF_RANGE, UNEXP_RENAL_NORMAL, UNEXP_WHICH_EXCEED
- Signals: internal source format leaked
- Description: Interdire dans le texte final toute source brute type doc_id/chunk_id, et convertir systématiquement vers liens viewer markdown.
- Target files: scripts/generation/generate_answer.py, scripts/generation/answer_validator.py, scripts/generation/source_normalization.py
- Effort: S
- Expected gain: +3 pass points

### 4. Brancher une route explicite treatment_refusal (pas diagnostic_refusal générique) (`F5_TREATMENT_REFUSAL_PATH`)
- Impact score: 5
...
### 5. Clarification déterministe pour requêtes analyte sans scope explicite (`F2_AMBIGUITY_CLARIFICATION_RENDERER`)
- Impact score: 4
