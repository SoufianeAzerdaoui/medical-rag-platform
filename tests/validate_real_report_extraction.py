...
def main() -> int:
    ...
    add_issue(issues, patient.get("ip_patient") == "53", "patient.ip_patient should be 53")
    add_issue(issues, patient.get("patient_id") == "53", "patient.patient_id should be 53")
    add_issue(issues, patient.get("patient_id_source_label") == "IP Patient", "patient.patient_id_source_label should be IP Patient")
    ...
    add_issue(issues, all(not str(img.get("file_path", "")).startswith("/home/") for img in document.get("images", [])), "image file_path should not be absolute /home/... paths")
    ...
    add_issue(issues, all(isinstance(result.get("source_line_start"), int) for result in results), "each result should contain source_line_start (int)")
    add_issue(issues, all(isinstance(result.get("source_line_end"), int) for result in results), "each result should contain source_line_end (int)")
    add_issue(
        issues,
        all(int(result.get("source_line_start")) <= int(result.get("source_line_end")) for result in results if isinstance(result.get("source_line_start"), int) and isinstance(result.get("source_line_end"), int)),
        "result source_line_start should be <= source_line_end",
    )
    for forbidden in [
        "reference_range",
        "unit",
    ...
    if logical_table:
        ...
        add_issue(issues, "result_kind" in logical_table.get("columns", []), "logical table should expose result_kind")
        add_issue(issues, "csv_path" not in logical_table, "logical table should omit csv_path when not produced")
        add_issue(issues, "json_path" not in logical_table, "logical table should omit json_path when not produced")
        ...
        add_issue(
            issues,
            all("result_kind" in record for record in logical_table.get("records", [])),
            "logical table records should include result_kind",
        )
        add_issue(
            issues,
    ...
    add_issue(issues, "edit_date" not in validation, "validation should not expose edit_date (use edited_date)")
    add_issue(issues, validation.get("edited_date") == "2024-06-04T12:04:25", "validation.edited_date mismatch")
